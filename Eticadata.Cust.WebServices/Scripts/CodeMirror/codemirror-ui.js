...
CodeMirrorUI.prototype = {
    ...
    initialize: function (textarea, options, mirrorOptions) {
        ...
        this.buttonDefs = {
            'search': [getMsgTranslated("Admin", "Procurar/Substituir"), "find_replace_popup", this.options.imagePath + "/find.png", this.find_replace_popup],
            'searchClose': [getMsgTranslated("Admin", "Fechar"), "find_replace_popup_close", "../../../../Imagens/Icons/Geral/Comandos/Eliminar.16.png", this.find_replace_popup_close],
            'searchDialog': [getMsgTranslated("Admin", "Procurar/Substituir"), "find_replace_window", this.options.imagePath + "/find.png", this.find_replace_window],
            'undo': [getMsgTranslated("Admin", "Anular Alteração"), "undo", this.options.imagePath + "/arrow_undo.png", this.undo],
            'redo': [getMsgTranslated("Admin", "Refazer Alteração"), "redo", this.options.imagePath + "/arrow_redo.png", this.redo],
            'jump': [getMsgTranslated("Admin", "Ir para a linha #"), "jump", this.options.imagePath + "/page_go.png", this.jump],
            'reindentSelection': [getMsgTranslated("Admin", "Formatar seleção"), "reindentSelect", this.options.imagePath + "/text_indent.png", this.reindentSelection],
            'reindent': [getMsgTranslated("Admin", "Formatar todo o documento"), "reindent", this.options.imagePath + "/page_refresh.png", this.reindent],
            'about': [getMsgTranslated("Admin", "Informações sobre o editor"), "about", this.options.imagePath + "/help.png", this.about]
        };
    ...
    createFindBar: function () {
        ...
        this.findString.size = 15;

        this.findButton = document.createElement("input");
        this.findButton.type = "button";
        this.findButton.className = "etiFindBarButton";
        this.findButton.value = getMsgTranslated("Admin", "Procurar");
        ...
        regLabel.title = getMsgTranslated("Admin", "Expressões Regulares");
        ...
        regLabel.appendChild(document.createTextNode("ExReg"));

        var caseLabel = document.createElement("label");
        caseLabel.title = getMsgTranslated("Admin", "Maiúsculas/minúsculas");
        ...
        this.replaceButton.className = "etiFindBarButton";
        this.replaceButton.value = getMsgTranslated("Admin", "Substituir");
        this.replaceButton.onclick = this.replace.cmuiBind(this);

        var replaceAllLabel = document.createElement("label");
        replaceAllLabel.title = getMsgTranslated("Admin", "Substituir Tudo");
        ...
        replaceAllLabel.appendChild(document.createTextNode(getMsgTranslated("Admin", "Tudo")));
    ...
    find: function (start) {
        ...
        if (findString == null || findString == '') {
            alert(getMsgTranslated("Admin", 'Deve introduzir um termo para pesquisar'));
            return;
        }
        ...
        if (found) {
            ...
            if (confirm(getMsgTranslated("Admin", "Não foram encontrados mais termos. Pretender regressar ao inicio?"))) {
                ...
                if (found) {
                    ...
                    alert(getMsgTranslated("Admin", "Não foram encontrados termos correspondentes"));
                }
            }
        }
    },
    replace: function () {
        ...
        if (findString != "" && findString != null) {
            ...
            alert(getMsgTranslated("Admin", "Deve pesquisar um termo para poder substituí-lo!"));
        }
    },
     ...
     */
    jump: function () {
        var line = prompt(getMsgTranslated("Admin", "Ir para a linha:"), "");
    ...
    about: function () {
        var string = getMsgTranslated("Admin", "Atalhos do teclado para ajuda:\n\n");
        string += "-> Ctrl-Space - " + getMsgTranslated("Admin", "Sugestões e completar código automaticamente") + " \n\n";
        string += "-> Ctrl-I - " + getMsgTranslated("Admin", "Mostrar o tipo de dados") + " \n\n";
        string += "-> Alt-. - " + getMsgTranslated("Admin", "Ir para a definição") + " \n\n";
        string += "-> Ctrl-Q - " + getMsgTranslated("Admin", "Alterar o nome") + " \n\n";
        string += "-> Ctrl-. - " + getMsgTranslated("Admin", "Selecionar elemento com o mesmo nome") + " \n\n";
        string += "-> Ctrl-Z - " + getMsgTranslated("Admin", "Anular alteração") + " \n\n";
        string += "-> Ctrl-Y - " + getMsgTranslated("Admin", "Refazer alteração") + " \n\n";
        alert(string);
    },
